...
const KEY = 'books';
// const GOOGLE_BOOKS = 'googleBooks';
...
// TODO: When user searches for a book, display the reults. each result has a 'add' butoon
// When adding a book, call a function that will send the google book to the service
// The service will create a new book obj (normal books - not google) and will add the info from the google book to the new object
// EXMAPLE:
// var newBook = {
//   title: volumeInfo.title
// }
// Add the new book to 'our' books

function save(book, bookReview) {
  _add(book, bookReview)

}

function _add(book, review) {
  if (!book.reviews) book.reviews = [];
  books.reviews.push(review)
}

function _createBooks() {
  var booksFromStorage = storageService.load(KEY)
...
function _saveBooksToStorage(booksFromStorage=books) {
  storageService.save(KEY, booksFromStorage)
}
